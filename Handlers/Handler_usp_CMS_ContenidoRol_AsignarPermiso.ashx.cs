...
namespace CMSBanchileSEGUROS
{
    public class Handler_usp_CMS_ContenidoRol_AsignarPermiso : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                IN_Handler_usp_CMS_ContenidoRol_AsignarPermiso entrada;
                using (var reader = new StreamReader(context.Request.InputStream))
                {
                    var body = reader.ReadToEnd();
                    entrada = JsonConvert.DeserializeObject<IN_Handler_usp_CMS_ContenidoRol_AsignarPermiso>(body ?? string.Empty);
                }

                if (entrada == null || entrada.idContenido <= 0 || entrada.idRol <= 0 || string.IsNullOrWhiteSpace(entrada.permiso))
                {
                    context.Response.StatusCode = 400;
                    context.Response.Write(JsonConvert.SerializeObject(new
                    {
                        CodigoRespuesta = "400",
                        GlosaRespuesta = "Debe proporcionar contenido, rol y permiso válidos."
                    }));
                    return;
                }

                var respuestaServicio = new RSP_Handler_usp_CMS_ContenidoRol_AsignarPermiso();
                try
                {
