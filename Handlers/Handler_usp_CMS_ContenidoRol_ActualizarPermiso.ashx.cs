...
namespace CMSBanchileSEGUROS
{
    public class Handler_usp_CMS_ContenidoRol_ActualizarPermiso : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                IN_Handler_usp_CMS_ContenidoRol_ActualizarPermiso entrada;
                using (var reader = new StreamReader(context.Request.InputStream))
                {
                    var body = reader.ReadToEnd();
                    entrada = JsonConvert.DeserializeObject<IN_Handler_usp_CMS_ContenidoRol_ActualizarPermiso>(body ?? string.Empty);
                }

                if (entrada == null || entrada.idContenidoRol <= 0 || string.IsNullOrWhiteSpace(entrada.permiso))
                {
                    context.Response.StatusCode = 400;
                    context.Response.Write(JsonConvert.SerializeObject(new
                    {
                        CodigoRespuesta = "400",
                        GlosaRespuesta = "Debe proporcionar un identificador y permiso válidos."
                    }));
                    return;
                }

                var respuestaServicio = new RSP_Handler_usp_CMS_ContenidoRol_ActualizarPermiso();
                try
                {
                    var instancia = new OperacionesBD();
                    respuestaServicio.Respuesta = instancia.usp_CMS_ContenidoRol_ActualizarPermiso(
                        entrada.idContenidoRol,
                        entrada.permiso);
