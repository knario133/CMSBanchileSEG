...
namespace CMSBanchileSEGUROS
{
    public class Handler_usp_CMS_Rol_Crear : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                IN_Handler_usp_CMS_Rol_Crear entrada;
                using (var reader = new StreamReader(context.Request.InputStream))
                {
                    var body = reader.ReadToEnd();
                    entrada = JsonConvert.DeserializeObject<IN_Handler_usp_CMS_Rol_Crear>(body ?? string.Empty);
                }

                if (entrada == null || string.IsNullOrWhiteSpace(entrada.nombreRol))
                {
                    context.Response.StatusCode = 400;
                    context.Response.Write(JsonConvert.SerializeObject(new
                    {
                        CodigoRespuesta = "400",
                        GlosaRespuesta = "El nombre del rol es obligatorio."
                    }));
                    return;
                }

                var respuestaServicio = new RSP_Handler_usp_CMS_Rol_Crear();
                try
                {
