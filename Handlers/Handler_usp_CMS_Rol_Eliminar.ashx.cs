...
namespace CMSBanchileSEGUROS
{
    public class Handler_usp_CMS_Rol_Eliminar : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                IN_Handler_usp_CMS_Rol_Eliminar entrada;
                using (var reader = new StreamReader(context.Request.InputStream))
                {
                    var body = reader.ReadToEnd();
                    entrada = JsonConvert.DeserializeObject<IN_Handler_usp_CMS_Rol_Eliminar>(body ?? string.Empty);
                }
                ...
                var respuestaServicio = new RSP_Handler_usp_CMS_Rol_Eliminar();
                try
                {
                    var instancia = new OperacionesBD();
                    respuestaServicio.Respuesta = instancia.usp_CMS_Rol_Eliminar(entrada.idRol);
