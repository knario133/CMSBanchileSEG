...
{
    public class Handler_usp_CMS_UsuarioRol_Asignar : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                IN_Handler_usp_CMS_UsuarioRol_Asignar entrada;
                using (var reader = new StreamReader(context.Request.InputStream))
                {
                    var body = reader.ReadToEnd();
                    entrada = JsonConvert.DeserializeObject<IN_Handler_usp_CMS_UsuarioRol_Asignar>(body ?? string.Empty);
                }

                if (entrada == null || entrada.idUsuario <= 0 || entrada.idRol <= 0)
                {
                    context.Response.StatusCode = 400;
                    context.Response.Write(JsonConvert.SerializeObject(new
                    {
                        CodigoRespuesta = "400",
                        GlosaRespuesta = "Debe proporcionar identificadores válidos de usuario y rol."
                    }));
                    return;
                }

                var respuestaServicio = new RSP_Handler_usp_CMS_UsuarioRol_Asignar();
                try
                {
                    var instancia = new OperacionesBD();
                    respuestaServicio.Respuesta = instancia.usp_CMS_UsuarioRol_Asignar(entrada.idUsuario, entrada.idRol);
        ...
        public class RSP_Handler_usp_CMS_UsuarioRol_Asignar
        {
